...
describe('GET /quizzes', () => {
  beforeEach(() => {
    loadQuizzes.mockResolvedValue(createMockQuizzes());
  });

  it('전체 퀴즈 목록을 반환한다', async () => {
    await request(app).get('/quizzes').expect(OK, createMockQuizzes());
  });
  ...
  it('random=true면 무작위 순서로 반환한다 (구성 원소는 동일)', async () => {
    ...
    expect(response.body).toHaveLength(createMockQuizzes().length);
    expect(response.body).toEqual(expect.arrayContaining(createMockQuizzes()));
  });
...
describe('GET /quizzes/:id', () => {
  ...
  it('존재하는 id면 해당 퀴즈를 반환한다', async () => {
    await request(app).get('/quizzes/1').expect(OK, createMockQuizzes()[0]);
  });
