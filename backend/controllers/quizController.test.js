...
const mockQuizzes = [
  {id: 1, type: 'OX퀴즈', question: '문제1', choices: ['O', 'X'], answer: 0, explanation: '해설1'},
  {id: 2, type: '객관식', question: '문제2', choices: ['1', '2', '3', '4'], answer: 1, explanation: '해설2'},
  {id: 3, type: '객관식', question: '문제3', choices: ['1', '2', '3', '4'], answer: 2, explanation: '해설3'}
];
...
describe('GET /quizzes', () => {
  beforeEach(() => {
    loadQuizzes.mockResolvedValue([...mockQuizzes]);
  });

  it('전체 퀴즈 목록을 반환한다', async () => {
    await request(app).get('/quizzes').expect(OK, mockQuizzes);
  });
  ...
  it('random=true면 무작위 순서로 반환한다 (구성 원소는 동일)', async () => {
    ...
    expect(response.body).toHaveLength(mockQuizzes.length);
    expect(response.body).toEqual(expect.arrayContaining(mockQuizzes));
  });
...
describe('GET /quizzes/:id', () => {
  ...
  it('존재하는 id면 해당 퀴즈를 반환한다', async () => {
    await request(app).get('/quizzes/1').expect(OK, mockQuizzes[0]);
  });
